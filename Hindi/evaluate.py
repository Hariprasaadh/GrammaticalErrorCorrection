...
def evaluate_model(model_path, test_file, output_file='evaluation_results.json'):
    ...
    df['Input sentence'] = df['Input sentence'].astype(str)
    df['Output sentence'] = df['Output sentence'].astype(str)
    
    sources = df['Input sentence'].tolist()
    references = df['Output sentence'].tolist()
    ...
    for pred, ref in zip(predictions, references):
        # GLEU (character-level for Hindi)
...
if __name__ == "__main__":
    model_path = '../Models/hindi_gec_mt5/best_model'
    test_file = '../Dataset/hindi/dev.csv'
